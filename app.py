# ==========================================
    ...
    if len(df_filtrado) > 0:
        # 1. Definimos las columnas a mostrar (incluimos url_archivo)
        columnas_mostrar = ['fecha_pago', 'Numero de predio', 'Nombre Completo', 'valor_pagado', 'url_archivo', 'Asociado?']
        
        # 2. Configuramos la tabla para que el link sea un botón clickable
        st.dataframe(
        ...
        if seleccion:
            ...
            if url_img:
                ...
                st.warning("Este registro no tiene una imagen asociada.")

    else:
        st.warning("No se encontraron pagos con los filtros seleccionados.")
